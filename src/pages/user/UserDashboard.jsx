...
import { Package, Heart, CreditCard, Loader2, ArrowRight } from 'lucide-react';
...
const UserDashboard = () => {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                // LocalStorage theke user nite hobe
                ...
                // Backend Route: /api/user/dashboard-summary/:userId
        ...
        <div className="animate-in fade-in duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                {[
                    ...
                    { 
                        ...
                        val: stats?.wishlistCount || '0', 
                        icon: <Heart className="text-rose-500" size={24}/>,
                        bg: 'bg-rose-50'
                    },
                ].map((stat, i) => (
                    <div key={i} className="bg-white p-8 border border-gray-100 shadow-sm rounded-3xl flex items-center justify-between hover:shadow-md transition-shadow">
                        <div>
            ...
            <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden">
                <div className="p-8 border-b border-gray-50 flex justify-between items-center">
                    <div>
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Last Order Status</p>
                        ...
                        <div className="flex items-center justify-between p-6 bg-gray-50/50 rounded-2xl border border-gray-100">
                            <div className="flex items-center gap-6">
                                <div className="w-16 h-16 bg-black text-white rounded-xl flex flex-col items-center justify-center font-black">
                                    ...
                                    <span className="text-lg">#{stats.recentOrder._id.slice(-4).toUpperCase()}</span>
                                </div>
                                <div>
                                    ...
                                    <p className="text-xs text-gray-400 font-bold tracking-tight">
                                        Placed on {new Date(stats.recentOrder.createdAt).toLocaleDateString('en-GB', {
                            ...
                            <div className="flex flex-col items-end gap-2">
                                <span className={`px-5 py-2 text-[10px] font-black rounded-full uppercase italic tracking-widest ${
                                ...
                                <p className="text-xs font-black">৳{stats.recentOrder.totalAmount}</p>
                            </div>
                        </div>
                    ) : (
                        <div className="text-center py-10">
                            <p className="text-gray-300 font-black uppercase tracking-widest text-sm">No recent orders to show</p>
                        </div>
                    )}
                </div>
            </div>
